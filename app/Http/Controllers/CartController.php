...
class CartController extends Controller
{
  ...
  public function checkOutApi(Request $request)
  {
    ...
    if (Auth::user()->checkProfile == null) {
      if ($request->is('api/*')) {
        return $this->makeJson('Harap lengkapi profil anda terlebih dahulu!', false, 400);
      }
      session()->flash('error', 'Harap lengkapi profil anda terlebih dahulu!');
      return redirect()->route('cart.list');
    }
    if (\Cart::session($userID)->getTotal() > (float)Auth::user()->sumAmount()) {
      if ($request->is('api/*')) {
        return $this->makeJson('Maaf, saldo tidak cukup!', false, 400);
      }
      session()->flash('error', 'Maaf, saldo tidak cukup!');
      return redirect()->route('cart.list');
    }
    ...
    if (count($cartItems) <= 0) {
      if ($request->is('api/*')) {
        return $this->makeJson('Maaf, tidak ada item di keranjang!', false, 400);
      }
      session()->flash('error', 'Maaf, tidak ada item di keranjang!');
      return redirect()->route('cart.list');
    }
    ...
    foreach ($cartItems as $cartItem) {
      $sub_total = floatval($cartItem->quantity) * floatval(env('HARGA_UNIT', 100000));
      $funding_id = $cartItem->id;
      $borrower_id = $cartItem->borrower_id;
      $borrower_user_id = $cartItem->borrower_user_id;
    ...
    if ($request->is('api/*')) {
      if ($transaction) {
        return $this->makeJson('Berhasil checkout!');
      } else {
